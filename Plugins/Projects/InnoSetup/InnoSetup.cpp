...
BOOL ParseFileISS(std::wstring sFileName, std::wstring &sInputFileNames, std::wstring &sOutputFileName);
...
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_Install(void) {
  wchar_t szInstallPath[MAX_PATH];
	...
	// Recherche automatique du programme Inno Setup
	GetEnvironmentVariable(L"ProgramFiles(x86)", szInstallPath, MAX_PATH);
	lstrcat(szInstallPath, L"\\Inno Setup 5");
	wsprintf(szProgName, L"%s\\Compil32.exe", szInstallPath);
	if (GetFileAttributes(szProgName) == -1) {
		...
		if (GetFileAttributes(szProgName) == -1) {
			...
			if (Len > 12 && !lstrcmpi(&szInstallPath[Len - 12], L"\\Compil32.exe")) {
...
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap20(void) {
	return (HBITMAP) LoadImage(HInstance, L"BMP_INNOSETUP_20", IMAGE_BITMAP, 20, 20, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap32(void) {
	return (HBITMAP) LoadImage(HInstance, L"BMP_INNOSETUP_32", IMAGE_BITMAP, 32, 32, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap48(void) {
	return (HBITMAP) LoadImage(HInstance, L"BMP_INNOSETUP_48", IMAGE_BITMAP, 48, 48, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_GetDescription(wchar_t *wszDescription, int SizeDescription) {
	lstrcpynW(wszDescription, L"Génération d'un programme d'installation", SizeDescription);
	return TRUE;
}
...
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_GetInputs(const wchar_t *szProjectFileName, const wchar_t *szProjectContent, const wchar_t **szInputFileNames) {
	...
	if (!ParseFileISS(szProjectFileName, sInputFileNames, sOutputFileName)) return FALSE;
...
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_Build(const wchar_t *szProjectFileName, const wchar_t *szProjectContent) {
	...
	if (GetInstallPath(szInstallPath, MAX_PATH)) {
		...
		wsprintf(szProgName, L"%s\\Compil32.exe", szInstallPath);
		wsprintf(szArgs, L" /cc \"%s\"", szProjectFileName);
		Ok = CreateProcessAndRedirectOut(szProgName, szArgs);

	}
	else {
		// Message d'erreur InstallPath non trouvé
		CallbackError(Translate(L"Programme \"Inno Setup\" pas ou mal installé").c_str(),
									Translate(L"Erreur").c_str(),
...
BOOL Button_ParcourirSourceClick(void) {
  ...
  ofn.lpstrFilter = L"Fichiers Inno Setup (*.iss)\0*.iss\0Tous (*.*)\0*.*\0";
...
BOOL GetInstallPath(wchar_t * szInstallPath, DWORD cbMaxInstallPath) {
  ...
  DWORD dwType;
	...
	Error = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Denis ROBERT\\FVCLIDE\\Plugins\\InnoSetup", 0,
                       KEY_READ, &hKey);
  ...
  if (Error == ERROR_SUCCESS) {
	...
	// Vérification de l'existence du programme InnoSetup
	if (Ok) {
		wsprintf(szProgName, L"%s\\ISCC.exe", szInstallPath);
...
BOOL SetInstallPath(wchar_t * szInstallPath) {
  ...
  Error = RegCreateKeyEx(HKEY_CURRENT_USER, L"Software\\Denis ROBERT\\FVCLIDE\\Plugins\\InnoSetup", 0, L"",
...
BOOL SelectInstallPath(wchar_t *szPath) {
  ...
  BrowseInfo.lpszTitle = L"Sélectionnez le chemin du programme Inno Setup Compil32.exe";
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
		...
		while (FileLineInput(hFile, &sBuffer)) {
			...
			if (!sBuffer.empty() && sBuffer[0] != ';') {
				...
				else if (sSection.compare("[Setup]") == 0) {

					Pos = (int) sBuffer.find_first_of("=");
					if (Pos != -1) {

						sKeyWord = SuppSpace(sBuffer.substr(0, Pos));
						sValue = SuppSpace(sBuffer.substr(Pos + 1, Len - Pos - 1));

						if (sKeyWord.compare("SetupIconFile") == 0) {
							sInputFileNames += Unicode(sValue);
							sInputFileNames += L"|";
						}
						else if (sKeyWord.compare("LicenseFile") == 0) {
							sInputFileNames += Unicode(sValue);
							sInputFileNames += L"|";
						}
				...
				else if (sSection.compare("[Files]") == 0) {

					while (!sBuffer.empty()) {

						Pos = (int) sBuffer.find_first_of(";");
						if (Pos != -1) {
							sCommand = SuppSpace(sBuffer.substr(0, Pos));
							sBuffer = SuppSpace(sBuffer.substr(Pos + 1, Len - Pos - 1));
						}
						else {
							sCommand = SuppSpace(sBuffer);
							sBuffer = "";
						}

						Pos = (int) sCommand.find_first_of(":");
						if (Pos != -1) {
							sKeyWord = SuppSpace(sCommand.substr(0, Pos));
							sValue = SuppSpace(sCommand.substr(Pos + 1, Len - Pos - 1));

							if (sKeyWord.compare("Source") == 0) {
								sValue = SuppQuotes(sValue);
								if (sValue.length() >= 2 &&
										(sValue.substr(sValue.length() - 2, 2).compare("\\*") == 0 ||
										 sValue.substr(sValue.length() - 2, 2).compare("/*") == 0)) {
