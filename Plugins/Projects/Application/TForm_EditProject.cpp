...
TForm_EditProject::TForm_EditProject(HWND hWnd, LPCTSTR szName)
  : TForm(hWnd, szName, FVCL_ARG_SUPP) {
	...
	GetListCompilers(&ListCompilers);
	for (i = 0; i < (int) ListCompilers.size(); i++) {
		ComboBox_Compilateur->Items->Add(GetNameCompiler(ListCompilers[i]));
	}

}

//---------------------------------------------------------------------------
// Accesseurs de la propriété TypeCompiler
//---------------------------------------------------------------------------

TYPECOMPILER TForm_EditProject::Get_TypeCompiler(void) {

	if (ComboBox_Compilateur->ItemIndex == -1) return tcUnknown;

	return ListCompilers[ComboBox_Compilateur->ItemIndex];
}

bool TForm_EditProject::Set_TypeCompiler(TYPECOMPILER NewTypeCompiler) {
	int i;


	for (i = 0; i < (int) ListCompilers.size(); i++) {
		if (NewTypeCompiler == ListCompilers[i]) {
			ComboBox_Compilateur->ItemIndex = i;
			return true;
		}
	}

	ComboBox_Compilateur->ItemIndex = -1;
...
void FASTCALL TForm_EditProject::Button_AjouterClick(TObject *Sender) {
	TTreeNode *FirstNode;
	TTreeNode *HeaderNode = NULL;
	TTreeNode *SourceNode = NULL;
	TTreeNode *ResourceNode = NULL;
	...
	if (OpenDialog_AddFile->Execute()) {
		...
		if (FirstNode) {
			HeaderNode = FirstNode->GetFirstChild();
			if (HeaderNode) {
				SourceNode = HeaderNode->GetNextSibling();
				if (SourceNode) {
					ResourceNode = SourceNode->GetNextSibling();
					if (ResourceNode) {
						OtherNode = ResourceNode->GetNextSibling();
					}
				}
			}
		}
		if (HeaderNode && SourceNode && ResourceNode && OtherNode) {
