...
BOOL LoadProject(const wchar_t *szProjectFileName, AnsiString *asGuid, AnsiString *asOutputFileName, TStringList *slListSrcFiles);
BOOL SaveProject(const wchar_t *szProjectFileName, AnsiString asGuid, AnsiString asOutputFileName, TStringList *slListSrcFiles);
...
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap20(void) {
	return (HBITMAP) LoadImage(HInstance, _T("BMP_APPLICATIONVHDL_20"), IMAGE_BITMAP, 20, 20, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap32(void) {
	return (HBITMAP) LoadImage(HInstance, _T("BMP_APPLICATIONVHDL_32"), IMAGE_BITMAP, 32, 32, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT HBITMAP WINAPI Plugin_GetBitmap48(void) {
	return (HBITMAP) LoadImage(HInstance, _T("BMP_APPLICATIONVHDL_48"), IMAGE_BITMAP, 48, 48, 0);
}

//---------------------------------------------------------------------------
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_GetDescription(wchar_t *aszDescription, int SizeDescription) {
	lstrcpynW(aszDescription, _TT("Application VHDL"), SizeDescription);
	return TRUE;
}
...
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_Open(wchar_t *szProjectFileName, wchar_t *szProjectContent, HWND hWnd) {
	...
	if (!LoadProject(szProjectFileName, &asGuid, &asOutputFileName, &slListSrcFiles)) return FALSE;
	...
	if (Form_EditProject->ShowModal() == mrOk) {
		...
		if (SaveProject(szProjectFileName, asGuid, asOutputFileName, &slListSrcFiles)) Ok = true;
	}
...
EXTERN_C DLLIMPORT BOOL WINAPI Plugin_AddFile(const wchar_t *szProjectFileName, const wchar_t *szProjectContent, const wchar_t *szFileName) {
	...
	if (!SaveProject(szProjectFileName, asGuid, asOutputFileName, &slListSrcFiles)) return FALSE;
...
BOOL CreateNewProject(AnsiString asProjectPath, AnsiString asProjectName, int ProjectType, AnsiString asOutputFileName) {
	...
	lstrcat(szModelePath, _T("\\..\\Models\\Projects\\vhdl"));
	lstrcat(szModelePath, _T("\\Empty project"));
	...
	if (CopyProject(szModelePath, asProjectPath, asProjectName, szGuid, &slListSrcFiles)) {
		...
		SaveProject(asProjectFileName.c_str(), szGuid, asOutputFileName, &slListSrcFiles);
	}
